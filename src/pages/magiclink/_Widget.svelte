<script lang="ts">
  ...
  const colorMap = {
    trello: "text-info bg-info/10",
    figma: "text-accent bg-accent/10",
    video: "text-error bg-error/10",
    doc: "text-warning bg-warning/10",
    other: "text-neutral bg-neutral/10",
  };
</script>
...
<div class="relative w-full max-w-md mx-auto">
  ...
  <div class="flex flex-col items-center mb-8 gap-4">
    ...
    <div
      class="bg-base-200/80 backdrop-blur-md p-1 rounded-2xl flex border border-base-content/5 shadow-inner"
    >
      <button
  ...
  <!-- Widget Container -->
  <div
    class="card-float bg-base-100 shadow-2xl border-none overflow-hidden p-0! rounded-[2.5rem]! flex flex-col min-h-[550px] transition-all duration-500 {isAdmin
  ...
  >
    ...
    <div class="p-8 pb-4">
      <div class="flex items-center gap-4 mb-8">
        <div
          class="w-14 h-14 rounded-full bg-wow flex items-center justify-center text-primary-content font-bold text-xl transition-transform duration-500 {isAdmin
            ? 'rotate-12'
            : ''}"
        >
        ...
        <div class="flex-1 min-w-0">
          {#if isAdmin}
            <input
              bind:value={title}
              class="bg-transparent border-none focus:ring-0 font-bold text-base-content text-lg w-full p-0 focus:outline-none placeholder:opacity-20"
            ...
            <div class="flex items-center gap-1">
              ...
              <input
                bind:value={subtitle}
                class="bg-transparent border-none focus:ring-0 text-[10px] text-success font-bold uppercase tracking-wider w-full p-0 focus:outline-none"
                placeholder="Status"
              />
            </div>
          {:else}
            <h4 class="font-bold text-base-content text-lg truncate">
              {title}
            </h4>
            <p
              class="text-[10px] text-success font-bold flex items-center gap-1 uppercase tracking-wider"
            >
        ...
        {#if !isAdmin}
          <div class="flex -space-x-2">
            <div
              class="w-8 h-8 rounded-full border-2 border-base-100 bg-base-200"
            ></div>
            <div
              class="w-8 h-8 rounded-full border-2 border-base-100 bg-primary text-primary-content text-[10px] flex items-center justify-center font-bold"
            >
              +2
            </div>
          </div>
      ...
      <div class="space-y-3">
        {#each links as link (link.id)}
          {#if isAdmin}
            <!-- Admin Resource Item -->
            <div
              class="flex flex-col p-4 rounded-2xl bg-base-200/50 border border-base-content/5 gap-3 group relative transition-all hover:bg-base-100 hover:shadow-md"
            >
              <div class="flex items-center gap-3">
                ...
                <div class="flex-1">
                  <input
                    ...
                    class="bg-transparent border-none focus:ring-0 font-bold text-sm text-base-content/70 p-0 h-5 w-full focus:outline-none"
          ...
          {:else}
            ...
            <a
              href={link.url}
              target="_blank"
              class="group relative flex items-center justify-between h-14 w-full rounded-2xl bg-base-200/50 hover:bg-base-100 hover:shadow-lg transition-all px-4 border border-transparent hover:border-base-content/5 no-underline"
            >
              <div class="flex items-center gap-4 flex-1 min-w-0">
                ...
                <span class="text-sm font-bold text-base-content/70 truncate"
                  >{link.title}</span
                >
              </div>
              <div class="flex items-center gap-2 shrink-0">
                ...
                ></div>
                <ExternalLink
                  class="w-3.5 h-3.5 text-base-content/10 group-hover:text-primary transition-colors"
                />
              </div>
            </a>
          {/if}
        ...
        {#if isAdmin}
          <button
            onclick={addLink}
            class="cursor-pointer w-full h-14 rounded-2xl border-2 border-dashed border-base-content/5 hover:border-primary/20 hover:bg-primary/5 transition-all flex items-center justify-center gap-2 text-sm font-bold opacity-30 hover:opacity-100 mt-4"
          >
            <Plus class="w-4 h-4" />
            Add Resource
          </button>
    ...
    <!-- Hub Footer -->
    <div class="mt-auto p-8 pt-4 border-t border-base-content/5 text-center">
      {#if isAdmin}
        <input
